...
Current bridge `0.20.2-race-safe-deploy` uses a vendor-derived V2.0.7 posture baseline (firmware gait selection, normal pace, body height 108, shoulder yaw 0, IMU off), keeps it stable across SDK steering updates, and deploys SDK as the single normal motor owner. It does **not** claim complete app-motion equivalence: app width 50 and SDK step 10 are different control domains and require powered measurement. The app backend is retained for compatibility, but a motion lease never crosses backends; the alternate backend is used only as a stop fallback after the active backend fails. Exact physical body height remains a commissioning measurement, not an offline assertion wearing a lab coat.
