...
The reactive controller was rewritten around a conservative full-body contract: six required sectors, 0.35 m occupancy-map inflation radius, no autonomous lateral escape or walking arcs, short supervised reverse backoff, full turn-sweep clearance, measured escape progress, repeated-action suppression, and fail-closed mission results. The first 0.30-second live reverse trial stopped with `escape_no_progress`: front-left clearance worsened from 0.412 m to 0.378 m and measured yaw changed about 9.6 degrees. Source `0.18.3-backoff-guard` added in-pulse worsening-clearance and 5-degree heading-drift aborts. Subsequent bounded forward trials completed without contact; the obstacle-response run stopped at the 0.65 m corridor, but a 0.30-second turn produced only about 2 degrees and correctly failed progress. Source `0.18.4-measured-turns` replaced exploration turn pulses with guarded 20-degree measured-yaw turns. Its first live turn stopped without contact when transient clearance reached 0.418 m. Source `0.18.5-supervised-reverse` added a 0.45 m turn-start margin and a gain-bounded reverse recovery, but its live trial yawed beyond 5 degrees after 1.04 seconds and gained only about 0.016 m. Source `0.18.6-yaw-corrected-reverse` segmented reverse motion and used guarded measured-yaw correction toward the original heading, but its first live recovery stopped when frontal clearance worsened. Source `0.18.7-pivot-fallback` added stopped fallback pivots, but the first live pivot showed that moving Cartographer yaw can regress during a gait cycle even when the settled post-stop pose rotated correctly. Source `0.18.8-stop-measure-turns` therefore controls turning through clearance-supervised 0.50-second micro-pivots and settled post-stop yaw measurements.
