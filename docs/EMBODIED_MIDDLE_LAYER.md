...
The reactive controller was rewritten around a conservative full-body contract: six required sectors, 0.35 m occupancy-map inflation radius, no autonomous lateral escape or walking arcs, short supervised reverse backoff, full turn-sweep clearance, measured escape progress, repeated-action suppression, and fail-closed mission results. The first 0.30-second live reverse trial stopped with `escape_no_progress`: front-left clearance worsened from 0.412 m to 0.378 m and measured yaw changed about 9.6 degrees. Source `0.18.3-backoff-guard` therefore aborts during reverse when frontal clearance worsens by more than 0.02 m or heading drifts by more than 5 degrees. A second physical trial remains pending manual repositioning into an open controlled area.
